  ...
  useEffect(() => {
    const fetchActiveEvents = async () => {
      try {
        ...
        setError(err.message || 'Failed to load events; showing fallback list.')
        setEvents(fallbackEvents)
      } finally {
        setLoading(false)
      }
        ...
        <div className="container">
          ...
          <div className="row justify-content-center">
            ...
            {!loading && events.length > 0 && events.map((event, index) => (
              <div key={event.id} className="col-md-6 col-lg-4 mb-4" data-aos="zoom-in" data-aos-delay={(index + 1) * 100}>
                <div className="card event-card h-100 shadow-sm">
                  {event.imageUrl ? (
                    <img src={event.imageUrl} className="card-img-top" alt={event.title} />
                  ) : (
                    <div className="card-img-top bg-light d-flex align-items-center justify-content-center" style={{ height: '200px' }}>✨</div>
                  )}
