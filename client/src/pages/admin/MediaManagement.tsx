...
const MediaManagement = () => {
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      setSelectedItem(null);
      setSuccessMessage(
        `Gallery item ${!selectedItem.isActive ? "activated" : "deactivated"} successfully`,
      );
    } catch (err: any) {
  ...
  const shareAllMedia = async () => {
    try {
      const shareableMedia = sharingHelper.prepareShareData(gallery, {
            ...
            <div className="card-body p-4">
              <form onSubmit={handleSubmit}>
                ...
                <div className="d-flex gap-2 mt-4 pt-3 border-top">
                  <button
                    type="submit"
                    aria-label={editingItem ? "Save changes" : "Add media"}
                ...
                {gallery.length === 0 ? (
                  ...
                  gallery.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td>
                        <span
                          className={`badge rounded-pill px-3 py-2 ${item.isActive ? "bg-success-subtle text-success" : "bg-secondary-subtle text-secondary"}`}
                        >
      ...
      <ConfirmationModal
        ...
        message={
          action === "delete"
            ...
            : `Do you want to ${selectedItem?.isActive ? "hide" : "show"} "${selectedItem?.title}"?`
        }
