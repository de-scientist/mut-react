...
const PrayerRequestsManagement = () => {
  ...
  const getStatusConfig = (status: string) => {
    const configs: Record<string, { color: string, icon: any }> = {
      ...
      PRAYED_FOR: { color: 'bg-info-subtle text-info-emphasis', icon: <Heart size={14}/> },
      ANSWERED: { color: 'bg-success-subtle text-success-emphasis', icon: <CheckCircle size={14}/> },
    }
      ...
      <div className="container">
        ...
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-5 gap-3">
          <div>
            ...
            <p className="text-muted mb-0">Review and moderate community prayer requests.</p>
          </div>
          <button
        ...
        <div className="row g-4 mb-5">
          <div className="col-md-4">
            <div className="card border-0 shadow-sm rounded-4 p-4 bg-white border-start border-primary border-5">
               <h6 className="text-uppercase text-muted fw-bold small">Total Requests</h6>
          ...
          <div className="col-md-4">
            <div className="card border-0 shadow-sm rounded-4 p-4 bg-white border-start border-warning border-5">
               <h6 className="text-uppercase text-muted fw-bold small">Awaiting Prayer</h6>
          ...
          <div className="col-md-4">
            <div className="card border-0 shadow-sm rounded-4 p-4 bg-white border-start border-success border-5">
               <h6 className="text-uppercase text-muted fw-bold small">Testimonies / Answered</h6>
        ...
        {error && (
          <div className="alert alert-danger border-0 shadow-sm alert-dismissible fade show mb-4" role="alert">
            {error}
            <button type="button" className="btn-close" onClick={() => setError(null)} aria-label="Dismiss"></button>
          </div>
        )}
                ...
                {requests.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center py-5">
                      <div className="opacity-25 mb-3"><MessageSquare size={48} /></div>
                      ...
                      <tr key={req.id}>
                        <td className="px-4">
                          <div className="d-flex align-items-center gap-2">
                            <div className="bg-light p-2 rounded-circle"><User size={16}/></div>
                            <span className="fw-bold">{req.name || 'Anonymous'}</span>
                          </div>
                        </td>
                        <td>
                        ...
                        <td>
                          <span className={`badge rounded-pill d-inline-flex align-items-center gap-1 ${req.isPublic ? 'bg-primary-subtle text-primary' : 'bg-light text-muted border'}`}>
                            {req.isPublic ? <Eye size={12}/> : <EyeOff size={12}/>}
                            {req.isPublic ? 'Public' : 'Private'}
                          </span>
                        </td>
                        <td>
                          <span className={`badge rounded-pill px-3 py-2 d-inline-flex align-items-center gap-1 ${config.color}`}>
                            {config.icon}
