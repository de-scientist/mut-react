...
export const registerMember = async (req: Request, res: Response) => {
  try {
    const { name, email, yearOfStudy, course, ministry1, ministry2, message } = req.body

    // Check if member with this email already exists
    const existingArr = await db.select().from(members).where(eq(members.email, email.trim())).limit(1)
    const existingMember = existingArr[0]

    if (existingMember) {
      return errorResponse(res, 'A member with this email has already registered', 409)
    }
    ...
    return successResponse(res, member, 'Member registration submitted successfully. We will review your application soon.', 201)
  } catch (error: any) {
    ...
    return errorResponse(res, error.message || 'Failed to submit member registration', 500)
  }
}
...
export const getMembers = async (req: Request, res: Response) => {
  try {
    ...
    return paginatedResponse(res, membersList, pagination, 'Members retrieved successfully')
  } catch (error) {
...
export const updateMemberStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (!['PENDING', 'APPROVED', 'REJECTED'].includes(status)) {
      return errorResponse(res, 'Invalid status. Must be PENDING, APPROVED, or REJECTED', 400)
    }

    const updatedArr = await db.update(members)
    ...
    return errorResponse(res, error.message || 'Failed to update member status', 500)
  }
}
