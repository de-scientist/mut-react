...
export const createBlogSchema = z.object({
  body: z.object({
    ...
    excerpt: z.string().max(400, 'Excerpt is too long').optional().or(z.literal('')),
...
export const getBlogs = async (req: Request, res: Response) => {
  try {
    ...
    if (search) {
      ...
      filters.push(
        or(ilike(blogs.title, likeTerm), ilike(blogs.excerpt, likeTerm), ilike(blogs.tags, likeTerm)),
      )
    }
    ...
    return paginatedResponse(res, data, pagination, 'Blogs retrieved successfully')
  } catch (error) {
...
export const createBlog = async (req: Request, res: Response) => {
  try {
    const { title, slug, excerpt, content, featuredImage, author, status = 'draft', tags } = req.body
